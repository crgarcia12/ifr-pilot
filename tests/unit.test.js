...
test('VOR CDI deflects toward course (fly-toward-the-bar convention)', () => {
  ...
  function vorCdi(VOR, st){
    ...
    return clamp(-dev / 10, -1, 1); // Standard VOR: 10° full-scale
  }
  ...
  // Confirm the source uses standard VOR sensitivity (10°) or ILS (2.5°).
  assert.match(html, /fullScale\s*=\s*state\.ap\.aprOn\s*\?\s*2\.5\s*:\s*10/);
});
